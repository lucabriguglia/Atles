...
namespace Atles.Domain.Categories.Validators
{
    ...
    {
        public UpdateCategoryValidator(IQuerySender querySender, IPermissionSetRules permissionSetRules)
        {
            RuleFor(c => c.Name)
                .NotEmpty().WithMessage("Category name is required.")
                .Length(1, 50).WithMessage("Category name must be at least 1 and at max 50 characters long.")
                .MustAsync((c, p, cancellation) => querySender.Send(new IsCategoryNameUnique { SiteId = c.SiteId, Name = p, Id = c.Id }))
