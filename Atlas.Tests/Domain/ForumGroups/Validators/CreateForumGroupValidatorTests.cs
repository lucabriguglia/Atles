...
namespace Atlas.Tests.Domain.ForumGroups.Validators
{
    ...
    public class CreateForumGroupValidatorTests : TestFixtureBase
    {
        ...
        {
            var command = Fixture.Build<CreateForumGroup>().With(x => x.Name, string.Empty).Create();

            var forumGroupRules = new Mock<IForumGroupRules>();
            var permissionSetRules = new Mock<IPermissionSetRules>();

            var sut = new CreateForumGroupValidator(forumGroupRules.Object, permissionSetRules.Object);
        ...
        {
            var command = Fixture.Build<CreateForumGroup>().With(x => x.Name, new string('*', 101)).Create();
